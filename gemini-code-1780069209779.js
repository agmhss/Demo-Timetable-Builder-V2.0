 ...
 * Version: Ultimate Master Build + Strictly Sheet Mode + Full Tracker Sync
 */
...
// =========================================================
// 🌟 DATA SYNC & CLOUD SAVING ENGINE (Fixed)
// =========================================================
...
window.runCalculationEngine = function(renderGrid = true) {
    ...
    if (window.rawTrackerData && window.rawTrackerData.length > 1) {
        window.rawTrackerData.slice(1).forEach(row => {
            ...
            window.examDutyTracker[tName] = parseInt(row[2]) || 0; // 🌟 FIX: Reading Exam Counts from Col C
        });
    }
...
window.saveDutiesToCloud = async function() {
    ...
    if (mode === 'substitution') {
        ...
        // 🌟 FIX: Prevent Double Saving for same session
    ...
    try {
        ...
        if(resultText.includes("Success")) {
            ...
            // Update live memory so UI reflects the new count instantly
